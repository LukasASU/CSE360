	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						...
						//omitted the number check, since we want it to be alphabetic only
					nextState = 1;
			...
			case 1: 
				...
				// . -> State 2
				else if (currentChar == '.' || currentChar == '-' || currentChar == '_') {	// Check for . or - or _
					nextState = 2;
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName character may only contain the characters A-Z, a-z, 0-9, . (period), - (dash), or _ (underscore)\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
